...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check permission
    const hasPermission = await PermissionsService.canCreate(user, 'suppliers')
    if (!hasPermission) {
      return NextResponse.json({ error: 'Permission denied. You do not have permission to create suppliers.' }, { status: 403 })
    }
