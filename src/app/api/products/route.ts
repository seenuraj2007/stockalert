...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check permission
    const hasPermission = await PermissionsService.canCreate(user, 'products')
    if (!hasPermission) {
      return NextResponse.json({ error: 'Permission denied. You do not have permission to create products.' }, { status: 403 })
    }
